#!/usr/bin/env bash
#SBATCH --job-name=q35-pr33778-after
#SBATCH --account=r7n
...
#SBATCH --output=/shared/amdgpu/home/andy_luo_3v7/qwen35-pr33778-after-pr34005-%j.out

# If the portfolio remains below target after the isolated PR #34005 bracket,
# test PR #33778 independently at the same selected maxrun-1 concurrency.
set -euo pipefail
...
patch_path="$shared_root/sglang_pr33778_strided_target_verify_qkv_instrumented.patch"
patch_sha256="b2a90ad736b7c2d0795316d1c32c001c107cf7f6c9b01eed0cd68bcbc43fc202"

for required in \
  "$previous_stdout" "$pr35872_stdout" "$pr37113_stdout" "$client_manifest" \
...
if grep -Fq 'skip_pr34005 reason=provisional_target_numerically_met' \
  "$previous_stdout"; then
  printf '%s skip_pr33778 reason=provisional_target_numerically_met upstream=%s\n' \
...
previous_summary="$(recover_summary "$previous_stdout" pr34005-draft-extend-lm-head-prune)"
pr35872_summary="$(recover_summary "$pr35872_stdout" pr35872-rocm-topk1)"
pr37113_summary="$(recover_summary "$pr37113_stdout" pr37113-rocm-gdn-decode)"
for summary in "$previous_summary" "$pr35872_summary" "$pr37113_summary"; do
...
grep -Fqx "manifest=$previous_manifest" "$previous_decision" || {
  echo "PR #34005 decision is not bound to $previous_manifest" >&2
  exit 3
}
grep -Fqx "evaluation=$previous_evaluation" "$previous_decision" || {
  echo "PR #34005 decision is not bound to $previous_evaluation" >&2
  exit 3
}

if [[ "$(jq -r '.target_numerically_met' "$previous_evaluation")" == true ]]; then
  printf '%s skip_pr33778 reason=provisional_target_numerically_met evaluation=%s\n' \
...
[[ "$actual_patch_sha256" == "$patch_sha256" ]] || {
  echo "PR #33778 patch checksum mismatch: expected $patch_sha256 got $actual_patch_sha256" >&2
  exit 2
}

base_candidate_manifests="$first_ladder_manifest $maxrun5_manifest $client_manifest $pr37113_manifest $pr35872_manifest $previous_manifest"
printf '%s starting_pr33778 concurrency=%s previous_evaluation=%s\n' \
...
env \
  ...
  PATCH_LABEL=pr33778-strided-target-verify-qkv \
  EXPECTED_ROUTE_REGEX='GDN_TARGET_VERIFY_QKV_BACKEND layout=strided backend=triton' \
  DURATION="$duration" \
  CONC="$conc" \
